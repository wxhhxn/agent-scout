...
定时任务应由 Codex Scheduled Tasks 负责，而不是写进 Skill：
...
```text
使用 $agent-scout，研究库位于 /你的绝对路径/AgentScout。
搜索最近 14 个完整自然日，给出 5 项高信号 Agent 技术工作。
优先工业界 AI 实验室的一手技术来源，同时保持主题多样性。
输出中文摘要，保存 digest，更新 seen.jsonl；未经我明确选择不要归档。
```
